    ...
    int calculateMinimumHP(vector<vector<int>>& dungeon) {
        ...
        for(int i=r-1;i>=0;i--)
        {
            for(int j=c-1;j>=0;j--)
            {
                ...
                // value is minimum health to land on next - health need to stay
                
                dp[i][j] = max(1,val); 
                // minimum value required is 1. maximum is the +ve value we calculated.
            }
        }
        
        return dp[0][0];  
        // The first element contains the minimum health needed to rescue the princess.
